...
class AnalyticsService:
    ...
    async def analyze_competitor(
        ...
        performance_score = self._calculate_performance_score(
            activities, products, news_items
        )
    ...
    def _assess_competitor_risk(
        ...
        high_impact_negative = [
            ...
            if a.impact_level == 'critical' and 'shutdown' in a.title.lower()
        ]
        risk_factors += len(high_impact_negative) * 3
        
        # Negative sentiment in news
        negative_news_ratio = len([n for n in news_items if n.sentiment == 'negative']) / max(len(news_items), 1)
    ...
    def _calculate_performance_score(
        ...
        # Sentiment score
        if news_items:
            positive_ratio = len([n for n in news_items if n.sentiment == 'positive']) / len(news_items)
    ...
    async def generate_market_forecast(
        ...
        confidence_interval = (
            max(0, avg_growth - 1.96 * std_dev),
            avg_growth + 1.96 * std_dev
        )
    ...
    async def _generate_competitor_movement_insights(
        self,
        db: AsyncSession
    ) -> List[Insight]:
        ...
        result = await db.execute(
            select(CompetitorActivity).where(
                CompetitorActivity.activity_date >= recent_date
            )
        )
        ...
        for activity_type, count in activity_types.items():
            if count > 5:
                insights.append(Insight(
                    ...
                    confidence=0.8,
                    impact_level=RiskLevel.MEDIUM if activity_type == 'product_release' else RiskLevel.LOW,
                    actionable=True,
                    recommendations=[
    ...
    async def _generate_market_trend_insights(
        ...
        for trend in trends:
            insights.append(Insight(
                ...
                description=trend.description or f"New trend detected in {trend.industry or 'general market'}",
                confidence=trend.confidence_level == 'high' and 0.8 or 0.6,
                impact_level=RiskLevel.HIGH if trend.growth_rate and trend.growth_rate > 20 else RiskLevel.MEDIUM,
                actionable=True,
                recommendations=[
