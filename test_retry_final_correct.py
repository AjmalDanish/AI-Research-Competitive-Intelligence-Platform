...
async def test_timeout_retry():
    ...
    print("\nActual retries: " + str(attempt_count - 1) + " (expected: 2)")
    print("Status: CORRECT (timeout is retryable)" if attempt_count - 1 == 2 else "INCORRECT (wrong count)")
...
async def test_500_retry():
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == 1:
                ...
                print("Attempt 2: Simulating success after retry")
                successful = True
                return attempt_count
    ...
    print("\nResult: RETRIED " + str(attempt_count - 1) + " times (expected: 2)")
    print("Status: CORRECT (500 is retryable)" if attempt_count - 1 == 2 else "INCORRECT (wrong count)")
    return attempt_count - 1
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == 1:
                ...
                print("Attempt2: This should not execute")
                successful = True
                return attempt_count
...
async def main():
    ...
    if all_correct:
        ...
        print("  - Timeout: Retries with exponential backoff (2 retries)")
        print("  - HTTP 500: Retries with exponential backoff (2 retries)")
        print("  - HTTP 429: Retries with exponential backoff (2 retries)")
        print("  - Invalid URL: No retry (0 retries - permanent error)")
        print("  - robots.txt Denial: No retry (0 retries - policy decision)")
    else:
