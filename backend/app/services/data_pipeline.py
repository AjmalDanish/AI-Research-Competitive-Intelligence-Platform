...
class DataTransformer:
    ...
    def extract_entities(text: str) -> Dict[str, List[str]]:
        ...
        entities = {"companies": [], "locations": [], "organizations": [], "people": []}
...
class DataPipeline:
    ...
    async def process_scraped_data(
        self, scraped_data: List[ScrapedData], db: AsyncSession
    ) -> PipelineResult:
        """
        ...
        result = PipelineResult(
            pipeline_id=pipeline_id, status=PipelineStatus.RUNNING, start_time=datetime.utcnow()
        )
        ...
        try:
            ...
            for data in scraped_data:
                try:
                    ...
                    self.logger.info(f"Processed data from {data.url} - {data.title}")
    ...
    async def run_scheduled_pipeline(
        self, competitor_ids: List[int], db: AsyncSession
    ) -> PipelineResult:
        """
        ...
        scraping_config = ScrapingConfig(
            max_concurrent_requests=3, request_timeout=30, retry_attempts=2
        )
