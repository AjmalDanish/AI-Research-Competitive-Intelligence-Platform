...
async def test_timeout_retry():
    ...
    for attempt in range(max_retries + 1):
        try:
            if attempt == 0:
                ...
                retry_count = attempt
                print(f"   Status: SUCCESS after {retry_count} retries")
                return retry_count
                
        except TimeoutError as e:
            retry_count += 1
            print(f"   Attempt {attempt}: TimeoutError occurred - {str(e)}")
    ...
    print(f"\n   Result: RETRIED {retry_count} times (expected: 2)")
...
async def test_500_retry():
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            retry_count += 1
            print(f"   Attempt {attempt}: HTTP 500 error - {str(e)}")
...
async def test_429_retry():
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            retry_count += 1
            print(f"   Attempt {attempt}: HTTP 429 error - {str(e)}")
